...
- Funciones que se encuentran en otros software para procesamiento de datos raster.


- `function(x,...)` -> `x` es generalmente un `SpatRaster`


- `...` argumentos específicos de la `function`.


- Argumento `filename`, por defecto es un `character` vacio `“”`.


- Sí no se se especifíca un `filename` por defecto devuelve un raster que es guardado en memoria (RAM)
