...
## Tipos de funciones applys

La familia de apply (`apply`, `tapply`, `lapply`, `sapply`)

Forma elegante de `apply` una función a diferentes casos (`matrix`, `data.frame`, `list`)

Evita el uso de ciclos (`for`, `while`, etc)
...
::: {.column width="50%"}
```{r}
apply(m, 1, sum) #promedio para cada file
apply(m, 2, mean) #promedio para cada columna
```
:::
:::
...
d <- data.frame(name=c('Yi', 'Yi', 'Yi', 'Er', 'Er'), m)
d$v2[1] <- NA
d
```
